...
N = 16; % No. antenas
K = 4;  % No. users
L = 20; % Length of Communication frame
power = 10^(0/10);  % P_T
N_montecarlo = 100;
SNRdB = [-2:2:12];
...
for tt=1:N
    for jj=1:length(theta)
        a(tt,jj)=exp(j*pi*(tt-ceil((N)/2))*sin(theta(jj))); % Steering vector a
    end
end
...
for nn = 1:N_montecarlo
    H = (randn(K,N)+1j*randn(K,N))/sqrt(2); % Channel
    N_pbits = 2*K*L;
    msg_bits = randi([0,1],1,N_pbits);
    Y = reshape(QPSK_mapper(msg_bits),[K,L]);
    X_omni = Orthogonal_Com_Rad(H,Y,power); % Problem (8)
    X_dir = Arbitrary_Com_Rad(H,Y,power,F); % Problem (10)
    H_pinv = pinv(H);
    tt = trace(H_pinv*Y*Y'*H_pinv');
    ff = sqrt(L*power/tt);
    X_trdoff1 = tradeoff_comrad(rho,H,Y,power,X_omni);
    X_trdoff2 = tradeoff_comrad(rho,H,Y,power,X_dir);
    X_trdoff3 = tradeoff_comrad_per_ant(rho,H,Y,power,X_omni);
    X_trdoff4 = tradeoff_comrad_per_ant(rho,H,Y,power,X_dir);
