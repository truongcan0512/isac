...
for nn = 1:N_montecarlo
    ...
    X_omni = Orthogonal_Com_Rad(H,Y,power);
    X_dir = Arbitrary_Com_Rad(H,Y,power,F);
...
%     X_trdoff5 = tradeoff_comrad_CM(rou,H,Y,power,X_arbi);
    for ii = 1:length(SNRdB)
        ...
        W = sqrt(N0/2)*(randn(K,L)+1j*randn(K,L))/sqrt(2);
        Z_CR1 = H*X_omni+W;
        Z_CR2 = H*X_dir+W;
        Z_CR3 = H*X_trdoff1+W;
        Z_CR4 = H*X_trdoff2+W;
...
xlabel('\theta(deg)');
ylabel('Beampattern');
legend('ComRad-Omni','ComRad-Dir','ComRad-Tradeoff-Omni','ComRad-Tradeoff-Dir');